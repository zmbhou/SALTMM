...
class Test():
    ...
    def build(self,net_input,net_label,net_id):
        if "output" not in self.net:
            with tf.name_scope("placeholder"):
                ...
                self.net["id"] = net_id
                self.net["drop_prob"] = tf.Variable(1.0) #0.5for main, 1 for DSRG ##在测试的时候，不用drop out;
    ...
    def pred(self):
        if self.h is not None:
            ...
            label_size = tf.py_func(lambda x:x.shape[1:3],[self.net["input"]],[tf.int32,tf.int32])
    ...
    def predict(self):
        ...
        p.metrics_predict_tf_with_crf(multiprocess_num=50,crf_config=crf_config,scales=[0.5,0.75,1.0],use_max=True,output_dir="result")
        #p.metrics_predict_tf_with_crf(multiprocess_num=50,crf_config=crf_config,scales=[0.5,0.75,1.0],use_max=True,output_dir="resultval59.9")
        end_time = time.time()
        print("total time:%f" % (end_time - start_time))

if __name__ == "__main__":
    category_num = 21
    t = Test({"batch_size":1,"input_size":None,"epoches":1,"category_num":category_num,"model_path":"./model/resultP5","accum_num":1})
    t.predict()
