...
def single_crf_metrics(params):
    ...
    if output_dir is not None:
        #io.imsave(os.path.join(output_dir,"%s_img.png"%id_),img/256.0)
        io.imsave(os.path.join(output_dir,"%s_output.png"%id_),(np.argmax(featmap,axis=2)))
        #io.imsave(os.path.join(output_dir,"%s_output.png"%id_),dataset.label2rgb(np.argmax(featmap,axis=2)))
        #io.imsave(os.path.join(output_dir,"%s_label.png"%id_),dataset.label2rgb(label[:,:,0]))
        io.imsave(os.path.join(output_dir,"%s_pred.png"%id_),crf_pred)
        #io.imsave(os.path.join(output_dir,"%s_pred.png"%id_),dataset.label2rgb(crf_pred))
