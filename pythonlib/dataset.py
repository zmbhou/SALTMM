...
class dataset():
    def __init__(self,config={}):
        ...
        self.main_path = self.config.get("main_path",os.path.join("F:\DEEPLEARNING\LINGUOSHENGrefinenet-master/datasets/voc2012_trainval/"))
        #self.main_path = self.config.get("main_path",os.path.join("F:\DEEPLEARNING\LINGUOSHENGrefinenet-master\datasets//VOCdevkit//VOC2012///"))
        #F:\DEEPLEARNING\LINGUOSHENGrefinenet-master\datasets//VOCdevkit//VOC2012//
    ...
    def get_data_f(self):
        ...
        for one in self.categorys:
            with open(os.path.join("pascal","%s.txt" % one),"r") as f:
    ...
    def next_batch(self,category=None,batch_size=None,epoches=-1):
        ...
        def m(x):
            ...
            if category == "train":
                #img,label = self.image_preprocess(img,label,random_scale=True,flip=True,rotate=False) ##训练时候的代码
                img,label = self.image_preprocess(img,label,random_scale=False,flip=False,rotate=False) #测试时候代码
            else:
